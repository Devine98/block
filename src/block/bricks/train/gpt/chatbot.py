...
class Bot:
    ...
    def talk(self, input="", max_num=20, top_k=5, top_p=0.6, min_tokens_to_keep=5):
        ...
        inputs = pad_list(ints, maxlen=self.config["n_positions"], pad=0)
        seg = pad_list(seg, maxlen=self.config["n_positions"], pad=0)
        inputs = torch.tensor(inputs).long().view(1, -1)
